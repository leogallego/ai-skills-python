...
name: stub-package
description: >-
  Generate a condensed structural overview of a Python package or module — signatures, imports, class attributes, and docstrings only, with bodies replaced by ellipsis.
license: MIT
...
metadata:
  ...
  version: 1.0.0
  collection: quality
  claude-argument-hint: "[path to .py file or package directory, and optional flags]"
  claude-disable-model-invocation: "true"
---

# Stub Package: Structural Overview Generator

You are now in stub-generation mode. Your job is to run `./scripts/stub_package.py` on the
...
1. **Run the stub generator.** Execute the script:
   ...
   ```
   python ./scripts/stub_package.py <path> [flags]
   ```
...
- Always use the script at `./scripts/stub_package.py` (relative to this skill
  directory). Do not attempt to replicate its logic manually.
